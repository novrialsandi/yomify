		...
		<div className={`${isFullwidth ? 'w-full' : ''} flex flex-col space-y-2`}>
			...
			<div className={`relative ${label ? 'mt-1' : ''}`}>
				...
				<input
					...
					className={`flex w-full items-center rounded-lg border border-border/dark bg-background/componnent-input outline-0 hover:border-primary focus:border-active focus:ring-0 ${className} ${errorMsg ? 'border-error text-error' : ''} ${sizeDataClass[size]} ${hasIconLeft ? 'pl-12' : 'pl-4'} ${hasIconRight || isPasswordField ? 'pr-10' : 'pr-4'}`}
					required={isRequired}
				/>
				{isPasswordField && (
